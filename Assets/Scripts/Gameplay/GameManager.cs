	...
	namespace MinoGames.SnakeGame.Gameplay
	{
		public sealed class GameManager : MonoBehaviour
		{
			...
			[Range(1, 5)]
			[Tooltip("Default size of player snake ")]
			[SerializeField] private int _initialSnakeSize;

			[Tooltip("Number of extra players managed by AI ")]
			[Range(1, 5)]
			...
			[Range(0.5F, 100)]
			[Tooltip("The Board Size")]
			[SerializeField] private float _boardSize;

			[SerializeField] private Transform _board;
			[SerializeField] private Transform _grid;

			[Tooltip("Player colors by player order")]
			...
			private void OnPlayerHitWall(int playerId)
			{
				...
				if (playerId != 0)
					player.transform.position = GetRandomPointInBoxCollider();
			}
			...
			private async void ShowScore()
			{
				...
				for (var i = 0; i < _players.Count; i++) 
					ScoreController.OnPlayerScore.Invoke(i);
			}
			...
			private Vector2 GetRandomPointInBoxCollider()
			{
				...
				float randomX = Random.Range(gridBox.bounds.min.x, gridBox.bounds.max.x);

				// Generate a random Y coordinate within the grid
				float randomY = Random.Range(gridBox.bounds.min.y, gridBox.bounds.max.y);
